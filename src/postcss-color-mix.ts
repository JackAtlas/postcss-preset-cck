...
const getValueRegexp = (name: string) =>
  new RegExp('\\b' + name + '\\(([^()]+)\\)', 'g')
const getVarsRegexp = (name: string) =>
  new RegExp('\\b' + name + '\\(([^()]*\\([^()]*\\)[^()]*)+\\)', 'g')

function replaceValues(
  root: Root,
  fn: string,
  replace: (values: string) => string
) {
  root.replaceValues(
    getValueRegexp(fn),
    { fast: `${fn}(` },
    (_, values: string) => replace(values)
  )
  root.replaceValues(
    getValueRegexp(fn),
    { fast: `${fn}(` },
...
const plugin = () => {
  return {
    postcssPlugin: 'postcss-c-color-mix',
    Once(root: Root) {
